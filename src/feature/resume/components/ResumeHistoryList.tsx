...
export const ResumeHistoryCard: React.FC<ResumeHistoryCardProps> = ({ item, onPress, onDelete }) => {
  const theme = useAppTheme();
  const resumeStyles = createResumeStyles(theme);

  return (
    <TouchableOpacity style={resumeStyles.historyCard} onPress={onPress} activeOpacity={0.7}>
      <View style={resumeStyles.historyCardInner}>
        ...
        <View style={resumeStyles.historyCardInfo}>
          <Text style={resumeStyles.historyCardName}>{item.full_name}</Text>
          <Text style={resumeStyles.historyCardRole}>{item.target_role}</Text>
          <View style={resumeStyles.historyCardMeta}>
            {item.experience_level && (
              <View style={resumeStyles.metaPill}>
                <Text style={resumeStyles.metaPillText}>{item.experience_level}</Text>
              </View>
            )}
            {item.industry && (
              <View style={resumeStyles.metaPill}>
                <Text style={resumeStyles.metaPillText}>{item.industry}</Text>
              </View>
            )}
            {item.tone && (
              <View style={[resumeStyles.metaPill, resumeStyles.metaPillAccent]}>
                <Text style={[resumeStyles.metaPillText, resumeStyles.metaPillAccentText]}>{item.tone}</Text>
              </View>
            )}
          </View>
          {item.core_skills && item.core_skills.length > 0 && (
            <Text style={resumeStyles.historyCardSkills} numberOfLines={1}>
        ...
        <View style={resumeStyles.historyCardRight}>
          ...
          <TouchableOpacity onPress={onDelete} style={resumeStyles.historyDeleteBtn} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
...
export const ResumeHistoryList: React.FC<ResumeHistoryListProps> = ({
  ...
  if (history.length === 0) {
    return (
      <View style={resumeStyles.emptyState}>
        ...
        <TouchableOpacity style={resumeStyles.emptyAction} onPress={onBuildNew}>
  ...
  return (
    <FlatList
      ...
      renderItem={({ item }) => (
        <ResumeHistoryCard item={item} onPress={() => onSelect(item)} onDelete={() => onDelete(item.id)} />
      )}
